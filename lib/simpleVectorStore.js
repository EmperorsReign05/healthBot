...
class SimpleVectorStore {
  ...
  async search(query, k = 3) {
    ...
    try {
      ...
      // Create embedding for the query using Google with retries
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          queryVector = await embedOnce();
          if (queryVector.length) break;
        } catch (err) {
          console.warn(`Embedding retry ${attempt}/3 due to error:`, err?.message || err);
  ...
  async getRelevantContext(query, threshold = 0.3) {
    try {
      ...
      let picked = results.filter(r => r.score > threshold);
      // If too few above threshold, fall back to top-k for broader coverage
      if (picked.length < 10) picked = results.slice(0, 20);
      const pieces = picked.map(r => r.document.content);
      // Cap context size (~14000 chars) to allow richer synthesis
      ...
      console.error('getRelevantContext error:', e);
      return '';
    }
  }
}
